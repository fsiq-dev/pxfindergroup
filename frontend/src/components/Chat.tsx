...
export function Chat({ room, messages, currentPlayerId, onSendMessage }: ChatProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Members sidebar */}
      <div className="border-b border-pxg-dark-border p-3">
          ...
          {room.members.map((member) => (
            <div
              ...
              className={`flex items-center gap-2 px-2 py-1.5 rounded-lg ${
                member.id === currentPlayerId ? 'bg-pxg-red/10' : ''
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-white truncate">
                  ...
                  {member.id === currentPlayerId && (
                    <span className="text-pxg-red ml-1">(you)</span>
                  )}
                </p>
              ...
              >
                ...
                <div
                  className={`max-w-[85%] px-3 py-2 rounded-2xl text-sm ${
                    isMine
                      ? 'bg-pxg-red text-white rounded-tr-sm'
                      : 'bg-pxg-dark-hover text-gray-100 rounded-tl-sm'
                  }`}
                >
                  {msg.content}
                </div>
      ...
      {/* Input */}
      <form onSubmit={handleSend} className="border-t border-pxg-dark-border p-3 flex gap-2">
        <input
          ref={inputRef}
